        ...
        <div class="card-body">
            <form method="GET" action="{{ route('horarios.index') }}">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="aprendiz_id" class="form-control select2">
                            ...
                            @foreach($aprendices as $a)
                                <option value="{{ $a->aprendiz_id }}" {{ request('aprendiz_id')==$a->aprendiz_id?'selected':'' }}>
                                    {{ $a->apr_PrimerNombre }} {{ $a->apr_Apellidos }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="ficha" class="form-control select2">
                            ...
                            @foreach($fichas as $f)
                                <option value="{{ $f->ficha_codigo }}" {{ request('ficha')==$f->ficha_codigo?'selected':'' }}>
                                    {{ $f->ficha_codigo }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="instructor" class="form-control select2">
                            ...
                            @foreach($instructores as $i)
                                <option value="{{ $i->instructor }}" {{ request('instructor')==$i->instructor?'selected':'' }}>
                                    {{ $i->instructor }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-3">
                    ...
                    <a href="{{ route('horarios.index') }}" class="btn btn-secondary"><i class="fas fa-sync"></i> Limpiar</a>
                </div>
            </form>
        </div>
    </div>
            ...
            <table class="table table-bordered table-striped">
                ...
                @forelse($horarios as $h)
                    <tr>
                        ...
                        <td>{{ $h->instructor }}</td>
                        <td>{{ $h->ambiente }}</td>
                        <td>{{ $h->fecha_inicio ? $h->fecha_inicio->format('Y-m-d') : '' }}</td>
